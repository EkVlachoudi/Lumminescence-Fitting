...
Npars = 4  #number of parameters per peak

#Function
def fourPL(D, I, b, Dc, a):
     return I*(1-(np.real(lambertw(b* np.exp (b) * np.exp (-D/Dc)))/b)**a)

def Theo1(D, *par):
    TL = 0.0
    for i in range(Np):
        TL += fourPL(D, par[Npars * i + 0], par[Npars * i + 1], par[Npars * i + 2], par[Npars * i + 3])
    return TL
...
for i in range(N):
    P1[i] = fourPL(Dx[i], par[0], par[1], par[2], par[3])
