...
def CW_theo1(T, par):
    ...
    for i in range(Npeaks):
        cw += CW(par[Npeak_pars * i + 0], par[Npeak_pars * i + 1],
                   par[Npeak_pars * i + 2], T)

    # then add the background
    cw += BGR(par[Npeak_pars * Npeaks + 0], par[Npeak_pars * Npeaks + 1],
...
class fit_func():
    ...
    def __call__(self, npar, deriv, f, par, iflag):
        ...
        for i in range(self.N):
            d = self.CW_exp[i] - CW_theo1(self.T[i], pp)
            f.value = f.value + d * d / self.CW_exp[i]


def CWOSL(print_all_data = False):
    # Read parameters file
    dirpara = "/pyroot/CWOSL/"
    ...
    channel, Tx, CW_exp = np.loadtxt(infile,
                                     dtype=[('', 'i4'), ('', 'f8'),
                                            ('', 'f8')],
                                     usecols=(0, 1, 2),
                                     unpack=True)
    ...
    if print_all_data==0:
        for i in range(0,10):
            print("{} {} {}".format(channel[i], Tx[i], CW_exp[i])) # print first 10 data lines
        print("...\n")
        for i in range(10, 0, -1):
            print("{} {} {}".format(channel[i], Tx[i], CW_exp[i])) # print last 10 data lines
    else:
        for i in range(0, N):
            print("{} {} {}".format(channel[i], Tx[i], CW_exp[i])) # print all data lines   
    ...
    #Create canvas
    cv = TCanvas("cv_CWOSL_fit", "cv_CWOSL_fit")
    cv.SetFillColor(0)
    cv.SetLogy()
    ...
    ipar += 1
    mini.mnparm(ipar, "l1", peaks[1][0], peaks[1][1], peaks[1][2], peaks[1][3], ierflg)
    ipar += 1
    ...
    ipar += 1
    mini.mnparm(ipar, "l2", peaks[4][0], peaks[4][1], peaks[4][2], peaks[4][3], ierflg)
    ipar += 1
    ...
    CWtheo = np.zeros(10000, dtype=float)
    for i in range(N):
    ...
    i=0
    for i in range(N):
        FOM=round(100*(np.sum(abs(CW_exp[i]-CWtheo[i]))/(np.sum(CWtheo[i]))),3)
    ...
    g.SetMarkerStyle(21)
    g.SetMarkerSize(0.7)
    g.SetTitle(";T (s); CW OSL Intensity (a.u.)")
    g.GetYaxis().SetTitleOffset(1.25)

    #Plot the fitted
    fitted = TF1("fitted", CW_theo, Tx[0], Tx[N - 1], Npeaks * Npeak_pars + 2)
    ...
    gmulti = TMultiGraph()
    gmulti.SetTitle(";T (s);CW OSL Intensity(a.u.)")
    gmulti.Add(g, "p")
    gmulti.Add(g1, "l")
    gmulti.Add(g2, "l")
    gmulti.Add(g3, "l")
    ...
    dirout=str('/pyroot/CWOSL/out/' )
    filep=input('Output data file name: ')
    outname1 = str(dirout + filep+'.out')
    outputFile = open(outname1, 'w')
    for i in range(N):
        outputFile.write("{} {} {} {} {} {} {} {} {} \n" .format(round(Tx[i],3), round(CW_exp[i],3), round(CW_theo1(Tx[i],par),3), round(P1[i],3), round(P2[i],3), round(P3[i],3), round(P4[i],3), round(P5[i],3), round(PG[i]),3))
    outputFile.close()
    print("=========================================\n")

    dirpar = str('/CWOSL/para/')
    filepar = input('Output parameter file name: ')
    outname2 = str(dirpar + filepar +'.par')
    parputFile = open(outname2,'w')
    parputFile.write( "Theo={}, FOM={}, LL={}, LU={} " .format(round(OloTheo,3), FOM, LL, LU))
    parputFile.write("\n ************************************************************ \n")
    ...
    if apofasi==1:
        dirpeaks = str("/pyroot/CWOSL/para/")
