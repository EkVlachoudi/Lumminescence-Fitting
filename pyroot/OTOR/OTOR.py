...
#Function
def fourPL(R, Dc,Io,D):
	return (Io)*(1+((np.real(sc.lambertw((R-1)*np.exp(R-1-(D/(Dc))))))/(1-R)))
...
def TL_theo1(D, par):
    TL = 0.0
    for i in range(Np):
        TL += fourPL(par[Npars * i + 0], par[Npars * i + 1], par[Npars * i + 2], D)
    return TL
...
def OTOR(print_all_data = False):
    ...
    #1st peak
    ipar = 0
    mini.mnparm(ipar, "R", peaks[0][0], peaks[0][1], peaks[0][2], peaks[0][3], ierflg)
    ipar += 1
    mini.mnparm(ipar, "Dc", peaks[1][0], peaks[1][1], peaks[1][2], peaks[1][3], ierflg)
    ipar += 1
    mini.mnparm(ipar, "I0", peaks[2][0], peaks[2][1], peaks[2][2], peaks[2][3], ierflg)
    ...
    for i in range(N):
        P1[i] = fourPL(par[0], par[1], par[2], Dx[i])
